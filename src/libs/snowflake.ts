...
export class SnowflakeFactory {
    ...
    serialization(id: number): ISnowflake {
        ...
        return {
            ...
            type: Object.entries(Types).find(
                t => t[1] == (id & 0x3e0000) >> 17,
            )[0],
            flags,
            count: id & 0xfff,
        };
...
export const matchFlags = (flag: number, id: number): boolean => {
    const BwIDFlag = Object.entries(Flags)
        .filter(f => f[1] == (id & 0x1f000) >> 12)
        .map(f => f[1])
        .reduce((acc, cur) => acc | cur);
    if (flag == (flag & BwIDFlag)) {
        return true;
    }
    return false;
};
